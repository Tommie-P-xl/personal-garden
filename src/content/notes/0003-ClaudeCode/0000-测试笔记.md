...
## 目录
- [一、省流：一句 Prompt 让 CC 自己配好（含自动安装 pwsh）](#一省流一句-prompt-让-cc-自己配好含自动安装-pwsh)
- [二、强制 Claude Code 使用 PowerShell (pwsh)](#二强制-claude-code-使用-powershell-pwsh)
- [三、解决第三方 API 超时问题](#三解决第三方-api-超时问题)
- [四、开启多智能体团队协作](#四开启多智能体团队协作)
- [五、屏蔽非必要网络流量](#五屏蔽非必要网络流量)
- [六、提高 Claude Code 缓存命中率](#六提高-claude-code-缓存命中率)
- [七、完整 settings.json 配置参考](#七完整-settingsjson-配置参考)
- [附：Unix → PowerShell 命令速查](#附unix--powershell-命令速查)

---

## 一、省流：一句 Prompt 让 CC 自己配好（含自动安装 pwsh）

如果你不想手动修改配置文件，可以直接复制下面这段完整的 Prompt，粘贴到 Claude Code 主会话中。它会**自动检查并安装 pwsh**，然后把 `CLAUDE.md` 和 `settings.json` 的所有核心配置合并落地，且保留现有内容不覆盖。
...
## 二、强制 Claude Code 使用 PowerShell (pwsh)
...
```
*(注：如果使用上方的“一句话 Prompt”，CC 会自动帮你完成这一步。)*

### 手动配置步骤

#### 步骤 1：修改 `~/.claude/CLAUDE.md`
在文件顶部追加 PowerShell 规则（内容同上方“省流”Prompt 中的【1】）。

#### 步骤 2：修改 `~/.claude/settings.json`
合并环境变量、默认 Shell 和权限配置（内容同上方“省流”Prompt 中的【2】和【3】）。
> **临时需要 Bash 怎么办？** 在 PowerShell Tool 里嵌套调用：`& bash -c "你的命令"`，对短命令反而更快。

---

## 三、解决第三方 API 超时问题

### 环境变量：`"API_FORCE_IDLE_TIMEOUT": "0"`
...
> **注意**：多智能体并行会**成倍消耗 Token**，建议仅在处理极其复杂、需要多线并行的任务时开启，日常简单问答无需使用。

---

## 五、屏蔽非必要网络流量

### 环境变量：`"CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC": "1"`
...
## 六、提高 Claude Code 缓存命中率

### 环境变量：`"CLAUDE_CODE_ATTRIBUTION_HEADER": "false"`

**作用：关闭计费归因 Header。**

在使用第三方 API（国内模型、中转站）时，关闭 billing header 可大幅提升**跨会话**的缓存命中率，节省约 90% 费用，速度提升约 8 倍。

> **注意**：此配置主要优化**跨会话**场景（频繁开新会话、并行开发、定时任务），同一会话内的多轮对话不受影响。

---

## 七、完整 `settings.json` 配置参考

将“解决超时”、“多智能体”、“流量优化”、“提高缓存命中率”和“强制使用 PowerShell”的所有配置整合后的完整示例：
